...
Test coverage for contracts works as expected and is automatically enabled when
`go test` is running with coverage enabled. When not desired, it can be disabled
for any Executor by using relevant methods. Be aware that coverage data collected by
`go test` itself will not be saved because it will be replaced with coverage data
